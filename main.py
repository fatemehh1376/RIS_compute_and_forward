...
if __name__ == "__main__":
    ...
    parser.add_argument('--experiment_type', default='sum_rate_power',
                        choices=['custom', 'power', 'rsi_elements', 'learning_rate', 'decay'],
    ...
    parser.add_argument("--num_antennas", default=1, type=int, metavar='N', help='Number of antennas in the BS')
    parser.add_argument("--num_RIS_elements", default=20, type=int, metavar='N', help='Number of RIS elements')
    parser.add_argument("--num_users", default=2, type=int, metavar='N', help='Number of users')
    # parser.add_argument("--power_t", default=5, type=float, metavar='N',
    #                     help='Transmission power for the constrained optimization in dB')

    parser.add_argument("--num_time_steps_per_eps", default=1000, type=int, metavar='N',
                        help='Maximum number of steps per episode (default: 20000)')
    parser.add_argument("--num_eps", default=10, type=int, metavar='N',
                        help='Maximum number of episodes (default: 5000)')
    parser.add_argument("--awgn_var", default=1e-2, type=float, metavar='G',
                        help='Variance of the additive white Gaussian noise (default: 0.01)')
    ...
    parser.add_argument("--tau", default=1e-3, type=float, metavar='G',
                        help='Learning rate in soft/hard updates of the target networks (default: 0.001)')
    parser.add_argument("--lr", default=1e-3, type=float, metavar='G',
    ...
    file_name = f"{args.num_users}_{args.num_RIS_elements}"
    ...
    env = RIS_Environment.RIS_Environment(args.num_antennas, args.num_RIS_elements, args.num_users,
                                          Variance_AWGN=args.awgn_var)
    ...
    for s in range(len(env.SNR)):
        ...
        for eps in range(int(args.num_eps)):
            ...
            for t in range(int(args.num_time_steps_per_eps)):
                ...
                next_state, reward, Done, _ = env.step(action, s)
            ...
            SNR_reward.append(np.mean(eps_rewards))
            print("episode: ", eps)
        instant_rewards.append(np.mean(SNR_reward))

        # if done:
        #     print(
        #         f"\nTotal T: {t + 1} Episode Num: {episode_num + 1} Episode T: {episode_time_steps} Max. Reward: {max_reward:}\n")
        #
        #     # Reset the environment
        #     state, done = env.reset(), False
        #     episode_reward = 0
        ...
        #     # SNR_total.append(eps_SNR)
        #     print("len instant_rewards", len(instant_rewards))
        #
    np.save(f"./Learning Curves/{file_name}",
            instant_rewards)
    # np.save(f"./Learning Curves/{args.experiment_type}/{file_name}_SNR_{episode_num + 1}",
    #         SNR_total)
