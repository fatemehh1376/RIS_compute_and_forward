...
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim, K, R, N, device, max_action=1):
    ...
    def compute_power(self, a):
        # Normalize the power
        G_real = a[:, :self.K ** 2].cpu().data.numpy()
        G_imag = a[:, self.K ** 2:2 * self.K ** 2].cpu().data.numpy()

        # print("len G_real:", len(G_real[0]))
        # print("len G_imag:", len(G_imag[0]))
        G = G_real.reshape(G_real.shape[0], self.K, self.N) + 1j * G_imag.reshape(G_imag.shape[0], self.K, self.N)

        G_and_G_Hermitian = np.matmul(G, np.transpose(G.conj(), (0, 2, 1)))

        current_power_t = torch.sqrt(torch.from_numpy(np.real(np.trace(G_and_G_Hermitian, axis1=1, axis2=2)))).reshape(
            -1, 1).to(
            self.device)
    ...
    def compute_phase(self, a):
        ...
        Phi_real = a[:, 0:self.R].detach()
        Phi_imag = a[:, self.R:].detach()

        # return torch.mean(torch.abs(Phi_real), dim=1).reshape(-1, 1) * np.sqrt(2), torch.mean(torch.abs(Phi_imag),
        #                                                                                       dim=1).reshape(-1,
        #                                                                                                      1) * np.sqrt(
        #     2)
    ...
    def forward(self, state):
        ...
        a = torch.relu(self.l1(state.float()))

        # Apply batch normalization to each hidden layer's input
        a = self.bn1(a)
        ...
        # current_power_t = self.compute_power(a.detach()).expand(-1, 2 * self.K ** 2) / np.sqrt(
        #     self.Power_at_transmitter)

        real_normal, imag_normal = self.compute_phase(a.detach())

        real_normal = real_normal.expand(-1, self.R)
        imag_normal = imag_normal.expand(-1, self.R)

        division_term = torch.cat([real_normal, imag_normal], dim=1)
        # print("real_normal:", real_normal)
        # print("imag_normal:", imag_normal)
        # print("division_term:", division_term)
        # print("final_result:", self.max_action * a / division_term)
...
class DDPG(object):
    def __init__(self, state_dim, action_dim, K, R, N, max_action, actor_lr, critic_lr, actor_decay,
        ...
        self.actor = Actor(state_dim, action_dim, K, R, N, max_action=max_action, device=device).to(
            self.device)
